...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="se1" 
        select="document('/o:cantus.seckau1.se1/TEI_SOURCE')"/>-->
...
\beginnumbering
    ...
    <xsl:template match="t:metamark[not(descendant::text()) and not(@corresp)]"> 
                <xsl:for-each
                    select="$ra//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]">
                    <xsl:choose>
                        <xsl:when
                            test=".//text() = $vo//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]//text()">                           
                            <xsl:text>+\footnoteA{</xsl:text><xsl:apply-templates mode="leermetamarks"/><xsl:text>fnEnde}</xsl:text>
                        </xsl:when>
                        <xsl:when
                            test=".//text() != $vo//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]//text()">              
                            <xsl:text>+\footnoteA{</xsl:text><xsl:apply-templates mode="leermetamarks"/><xsl:text>fnEnde}</xsl:text>
                ...
                <xsl:for-each
                    select="$vo//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]">
                    <xsl:choose>
                        <xsl:when
                            test=".//text() = $ra//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]//text()"></xsl:when>
                        <xsl:otherwise>                
                            <xsl:text>+\footnoteA{</xsl:text><xsl:apply-templates mode="leermetamarks"/><xsl:text>fnEnde}</xsl:text>  
                        </xsl:otherwise>
                    </xsl:choose>                 
    ...
    <xsl:template match="t:metamark[not(t:pb) and normalize-space(.)]">
       ...
       -->
        ...
        <xsl:for-each select="$sal_sal//t:metamark[@xml:id = current()/@xml:id]">
            <xsl:variable name="current_text_sal" select="."/>
            <xsl:variable name="text_vo" select="$vo//t:metamark[@xml:id = current()/@xml:id]"/>
            <xsl:variable name="text_ra" select="$ra//t:metamark[@xml:id = current()/@xml:id]"/>

            <xsl:choose>

                <!--Wenn der Text übereinstimmt keine Fußnoten-->
                <xsl:when test="$current_text_sal = $text"/>
                ...
                <xsl:when test="descendant::t:del[@subtype = 'signingover']">
                    <xsl:choose>
                        <xsl:when
                            ...
                            <xsl:text xml:space="preserve"> SAL; </xsl:text>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:text>\textit{ras., deinde add.} </xsl:text>
                ...
                <xsl:when test="descendant::t:add[not(parent::t:del)][@type = 'marginal']">
                    <xsl:if
                        test="not($current_text_sal//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)] = $text//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)])">
                ...
                <!--<xsl:when test="not(normalize-space(.))"><xsl:text>\textit{om.}</xsl:text><xsl:text xml:space="preserve"> SAL; </xsl:text></xsl:when>-->

                <!--om.-->
                <xsl:when test="not(normalize-space(.))">
                    <!--Es wird auf Gleichheit der Wörter wieder geprüft, stimmen die Wörter überein wurde omn. bereits geschrieben!-->
                    <xsl:if
                        test="not($current_text_sal//t:metamark[@xml:id = current()/@xml:id] = $vo//t:metamark[@xml:id = current()/@xml:id])">
                ...
                <!--<xsl:when test="$current_text_sal=$text"></xsl:when>-->
                <!--Sebi: Habe ich an den Anfang verlegt!-->
                <xsl:when test="$current_text_sal = $text_ra">
                    <xsl:apply-templates/>
                    <xsl:text xml:space="preserve"> SAL; RA; </xsl:text>
                </xsl:when>
                <xsl:when test="$current_text_sal = $text_vo">
                    <xsl:apply-templates/>
                    <xsl:text xml:space="preserve"> SAL; VO; </xsl:text>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates/>
                    <xsl:text xml:space="preserve"> SAL; </xsl:text>
        ...
        <xsl:for-each select="$vo//t:metamark[@xml:id = current()/@xml:id]">
            <xsl:variable name="current_text_vo" select="."/>
            <xsl:variable name="text_sal" select="$sal_sal//t:metamark[@xml:id = current()/@xml:id]"/>
            <xsl:variable name="text_ra" select="$ra//t:metamark[@xml:id = current()/@xml:id]"/>
            ...
            <xsl:choose>
                <xsl:when test="descendant::t:del[@subtype = 'signingover']">
                    <xsl:choose>
                        <xsl:when
                            ...
                            <xsl:value-of
                                select="descendant::t:del[@subtype = 'signingover']/t:add/t:seg[@type = 'incipit']/substring(@ana, 2)"/>
                            <xsl:text xml:space="preserve"> VO; </xsl:text>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:text>\textit{ras., deinde add.} </xsl:text>
                            <xsl:value-of select="descendant::t:del[@subtype = 'signingover']"/>
                ...
                <xsl:when test="descendant::t:add[not(parent::t:del)][@type = 'marginal']">
                    ...
                    <xsl:if
                        test="not($vo//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)] = $sal_sal//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)])">
                        <xsl:text>\textit{add. in marg.} </xsl:text>
                        <xsl:value-of select="descendant::t:add[not(parent::t:del)]"/>
                    </xsl:if>
                    <xsl:text xml:space="preserve"> VO; </xsl:text>
                ...
                <xsl:when test="descendant::t:add[not(parent::t:del)]">
                    ...
                    <xsl:if
                        test="not($vo//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)] = $sal_sal//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)])">
                        <!--<xsl:value-of select="$vo//t:metamark[@xml:id=current()/@xml:id]//t:add[not(parent::t:del)]"/>-->
                        <!-- Test für if Bedingung: Das Funktioniert!!!-->
                        <xsl:text>\textit{superscript.} </xsl:text>
                        <xsl:value-of select="descendant::t:add[not(parent::t:del)]"/>
                    </xsl:if>
                    <xsl:text xml:space="preserve"> VO; </xsl:text>
                ...
                <xsl:when test="not(normalize-space(.))">
                    <!--Es wird auf Gleichheit der Wörter wieder geprüft, stimmen die Wörter überein wurde omn. bereits geschrieben!-->
                    <xsl:if
                        test="not($vo//t:metamark[@xml:id = current()/@xml:id] = $sal_sal//t:metamark[@xml:id = current()/@xml:id])">
                        <xsl:text>\textit{om.}</xsl:text>
                    </xsl:if>
                    <xsl:text xml:space="preserve"> VO; </xsl:text>
                    <!--VO wird immer geschrieben (bzw. basierend auf obige Bedingung)-->
                </xsl:when>
                <!--om. end-->

                <xsl:when test="$current_text_vo = $text_ra">
                    <xsl:apply-templates/>
                    <xsl:text xml:space="preserve"> VO; RA; </xsl:text>
                </xsl:when>
                <xsl:when test="$current_text_vo = $text_sal"/>
                <xsl:when test="$current_text_vo = $text"/>
                <xsl:otherwise>
                    <xsl:apply-templates/>
                    <xsl:text xml:space="preserve"> VO; </xsl:text>
                </xsl:otherwise>
        ...
        <xsl:for-each select="$ra//t:metamark[@xml:id = current()/@xml:id]">
            <xsl:variable name="current_text_ra" select="."/>
            <xsl:variable name="text_sal" select="$sal_sal//t:metamark[@xml:id = current()/@xml:id]"/>
            <xsl:variable name="text_vo" select="$vo//t:metamark[@xml:id = current()/@xml:id]"/>
            ...
            <xsl:choose>
                ...
                <xsl:when test="descendant::t:del[@subtype = 'signingover']">
                    <xsl:choose>
                        <xsl:when
                            ...
                            <xsl:value-of
                                select="descendant::t:del[@subtype = 'signingover']/t:add/t:seg[@type = 'incipit']/substring(@ana, 2)"/>
                            <xsl:text xml:space="preserve"> RA; </xsl:text>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:text>\textit{ras., deinde add.} </xsl:text>
                            <xsl:value-of select="descendant::t:del[@subtype = 'signingover']"/>
                ...
                <xsl:when test="descendant::t:add[not(parent::t:del)][@type = 'marginal']">
                    <!--If values from previous iterations are not identical to the current one, display "superscript"-->
                    <xsl:if
                        test="not($ra//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)] = $vo//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)]) and not($ra//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)] = $sal_sal//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)])">
                        <xsl:text>\textit{add. in marg.} </xsl:text>
                        <xsl:value-of select="descendant::t:add[not(parent::t:del)]"/>
                    </xsl:if>
                    <xsl:text xml:space="preserve"> RA; </xsl:text>
                ...
                <!--<xsl:when test="string-join(.//text()[normalize-space()],'+')=$text_sal_sal"><xsl:text> RA; </xsl:text></xsl:when>-->
                <!--<xsl:when test="./t:pb/@n=$pb"></xsl:when>
                <xsl:when test="t:pb"><xsl:text>(</xsl:text><xsl:value-of select="./t:pb/@n"/><xsl:text>)</xsl:text><xsl:text xml:space="preserve"> SP1;</xsl:text></xsl:when>-->
                ...
                <xsl:when test="not(normalize-space(.))">
                    <!--Es wird auf Gleichheit der Wörter wieder geprüft, stimmen die Wörter überein wurde omn. bereits geschrieben!-->
                    <xsl:if
                        test="not($ra//t:metamark[@xml:id = current()/@xml:id] = $vo//t:metamark[@xml:id = current()/@xml:id]) and not($ra//t:metamark[@xml:id = current()/@xml:id] = $sal_sal//t:metamark[@xml:id = current()/@xml:id])">
                        <xsl:text>\textit{om.}</xsl:text>
                    </xsl:if>
                    <xsl:text xml:space="preserve"> RA; </xsl:text>
                    <!--RA wird immer geschrieben (bzw. basierend auf obige Bedingung)-->
                </xsl:when>
                <!--om. end-->

                <xsl:when test="$current_text_ra = $text_vo"/>
                <xsl:when test="$current_text_ra = $text"/>
                <xsl:otherwise>
                    <xsl:apply-templates/>
                    <xsl:text xml:space="preserve"> RA; </xsl:text>
                </xsl:otherwise>
    ...
    <xsl:template match="t:div[preceding-sibling::t:head][normalize-space(.)]">
    ...
    <xsl:template match="t:ab[@ana = '#head']">
        <xsl:if test="string-length() > 0">
            <xsl:if test="t:seg[@type = 'incipit']">
                <xsl:value-of select="t:seg[@type = 'incipit']"/>
                <xsl:text> </xsl:text>
            </xsl:if>
            <xsl:choose>
                <xsl:when test="./t:note[@type = 'supplied']">
                    <xsl:text>\textbf{&lt;</xsl:text>
                    <xsl:value-of select="./t:label/normalize-space()"/>
                    <xsl:text>&gt;}</xsl:text>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:text>\textbf{</xsl:text>
