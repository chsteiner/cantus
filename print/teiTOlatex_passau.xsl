...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:output method="text" indent="yes" encoding="utf8"/>
      <xsl:variable name="sve"
          select="document('passau_sve_current_tei.xml')"/>
    <xsl:variable name="knb"
        select="document('passau_knb_current_tei.xml')"/>
    <xsl:variable name="sal"
        select="document('passau_sal_current_tei.xml')"/>
    <xsl:variable name="wie"
        select="document('passau_wie_current_tei.xml')"/>
...
\end{document}
    ...
    <xsl:template match="t:metamark[not(descendant::text()) and not(@corresp)]"> 
                <xsl:for-each
                    select="$knb//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]">
                    <xsl:choose>
                        <xsl:when
                            test=".//text() = $sve//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]//text()">                           
                            <xsl:text> !!S!! </xsl:text><xsl:apply-templates/><xsl:text> =KNB,SVE= !!E!! </xsl:text>
                        </xsl:when>
                        <xsl:when
                            test=".//text() != $sve//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]//text()">              
                            <xsl:text> !!S!! </xsl:text><xsl:apply-templates/><xsl:text> =KNB= !!E!! </xsl:text>
                        </xsl:when>
                        <xsl:otherwise>   
                            <xsl:text> !!S!! </xsl:text><xsl:apply-templates/><xsl:text> =KNB= !!E!! </xsl:text>           
                        </xsl:otherwise>
                ...
                <xsl:for-each
                    select="$sve//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]">
                    <xsl:choose>
                        <xsl:when
                            test=".//text() = $knb//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]//text()"></xsl:when>
                        <xsl:otherwise>                
                            <xsl:text> !!S!! </xsl:text><xsl:apply-templates/><xsl:text> =SVE= !!E!! </xsl:text>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:for-each>
                <xsl:for-each
                    select="$wie//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]">   
                    <xsl:text> !!S!! </xsl:text><xsl:apply-templates/><xsl:text> =WIE= !!E!! </xsl:text> 
                </xsl:for-each>
                <xsl:for-each
                    select="$sal//t:metamark[@xml:id = current()/@xml:id][normalize-space(.)]">            
                    <xsl:text> !!S!! </xsl:text><xsl:apply-templates/><xsl:text> =SAL= !!E!! </xsl:text> 
                </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template match="t:metamark[not(t:pb) and normalize-space(.)]">
        ...
        <xsl:for-each select="$sve//t:metamark[@xml:id = current()/@xml:id]">
            <xsl:variable name="current_text_sve" select="."/>
            <xsl:variable name="text_wie" select="$wie//t:metamark[@xml:id = current()/@xml:id]"/>
            <xsl:variable name="text_knb" select="$knb//t:metamark[@xml:id = current()/@xml:id]"/>
            <xsl:variable name="text_sal" select="$sal//t:metamark[@xml:id = current()/@xml:id]"/>
            ...
            <xsl:choose>
                <xsl:when test="descendant::t:del[@subtype = 'signingover']">
                    <xsl:choose>
                        <xsl:when
                            ...
                            <xsl:text xml:space="preserve"> SVE, </xsl:text>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:text>\textit{ras., deinde add.} </xsl:text>
                ...
                <xsl:when test="descendant::t:add[not(parent::t:del)][@type = 'marginal']">
                    ...
                    <xsl:choose>
                        <xsl:when
                            test="$sve//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)][@type = 'marginal'] = $knb//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)][@type = 'marginal']">
                            <xsl:text>\textit{add. in marg.} </xsl:text>
                            <xsl:value-of select="descendant::t:add[not(parent::t:del)][@type = 'marginal']"/>
                            <xsl:text xml:space="preserve"> KNB,SVE, </xsl:text>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:text>\textit{add. in marg.} </xsl:text>
                            <xsl:value-of select="descendant::t:add[not(parent::t:del)][@type = 'marginal']"/>
                ...
                <xsl:when test="descendant::t:add[not(parent::t:del)]">
                    ...
                    <xsl:choose>
                        <xsl:when
                            test="$sve//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)] = $knb//t:metamark[@xml:id = current()/@xml:id]//t:add[not(parent::t:del)]">
                ...
                <xsl:when test="not(normalize-space(.))">
                    ...
                    <xsl:choose>
                        <xsl:when test="$sve//t:metamark[@xml:id = current()/@xml:id] = $knb//t:metamark[@xml:id = current()/@xml:id]">
                        <xsl:text>\textit{om.} KNB, SVE, </xsl:text>
                        </xsl:when>
                        <xsl:otherwise><xsl:text>\textit{om.} SVE, </xsl:text></xsl:otherwise>
                    </xsl:choose>
                </xsl:when>
                <!--om. end-->

                <xsl:when test="$current_text_sve = $text_knb">
                    <xsl:apply-templates/>
                    <xsl:text xml:space="preserve"> SVE,KNB, </xsl:text>
                </xsl:when>
                <xsl:when test="$current_text_sve = $text_wie"/>
                <xsl:when test="$current_text_sve = $text_sal"/>
                <xsl:when test="$current_text_sve = $text"/>
                <xsl:otherwise>
                    <xsl:apply-templates/>
                    <xsl:text xml:space="preserve"> SVE, </xsl:text>
                </xsl:otherwise>
        ...
        <xsl:for-each select="$knb//t:metamark[@xml:id = current()/@xml:id]">
            <xsl:variable name="current_text_knb" select="."/>
            <xsl:variable name="text_wie" select="$wie//t:metamark[@xml:id = current()/@xml:id]"/>
            <xsl:variable name="text_sve" select="$sve//t:metamark[@xml:id = current()/@xml:id]"/>
            <xsl:variable name="text_sal" select="$sal//t:metamark[@xml:id = current()/@xml:id]"/>
            ...
            <xsl:choose>
                <xsl:when test="descendant::t:del[@subtype = 'signingover']">
                    <xsl:choose>
                        <xsl:when
                            ...
                            <xsl:value-of
                                select="descendant::t:del[@subtype = 'signingover']/t:add/t:seg[@type = 'incipit']/substring(@ana, 2)"/>
                            <xsl:text xml:space="preserve"> KNB, </xsl:text>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:text>\textit{ras., deinde add.} </xsl:text>
                            <xsl:value-of select="descendant::t:del[@subtype = 'signingover']"/>
                ...
                <xsl:when test="not(normalize-space(.))">
                    <!--Es wird auf Gleichheit der Wörter wieder geprüft, stimmen die Wörter überein wurde omn. bereits geschrieben!-->       
                    <xsl:choose>
                        <xsl:when test="$sve//t:metamark[@xml:id = current()/@xml:id] = $knb//t:metamark[@xml:id = current()/@xml:id]"></xsl:when>
                        <xsl:otherwise><xsl:text>\textit{om.} KNB, </xsl:text></xsl:otherwise>
                    </xsl:choose>
                </xsl:when>
                <!--om. end-->

                <xsl:when test="$current_text_knb = $text_sve"/>
                <xsl:when test="$current_text_knb = $text"/>
                <xsl:otherwise>
                    <xsl:apply-templates/>
                    <xsl:text xml:space="preserve"> KNB, </xsl:text>
                </xsl:otherwise>
